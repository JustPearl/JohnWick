...
export class Game {
  ...
  private buildViewModels() {
    const dark = new THREE.MeshToonMaterial({ color: 0x1c2226 });
    const steel = new THREE.MeshToonMaterial({ color: 0x4d5a60 });
    const amber = new THREE.MeshToonMaterial({ color: 0xffb03a, emissive: 0xff8a2a, emissiveIntensity: 0.5 });
    const grip = new THREE.MeshToonMaterial({ color: 0x2b2320 });
    ...
    const build = (fn: (g: THREE.Group) => THREE.Vector3) => {
    ...
    // WIDOW-9 pistol
    build((g) => {
      const slide = new THREE.Mesh(new THREE.BoxGeometry(0.075, 0.09, 0.34), steel);
      slide.position.set(0, 0.05, -0.08);
      const frame = new THREE.Mesh(new THREE.BoxGeometry(0.065, 0.06, 0.26), dark);
      frame.position.set(0, -0.02, -0.04);
      const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.1, 8), dark);
      barrel.rotation.x = Math.PI / 2;
      barrel.position.set(0, 0.05, -0.28);
      const gr = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.14, 0.08), grip);
      gr.position.set(0, -0.1, 0.05);
      gr.rotation.x = 0.28;
      const sight = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.03, 0.02), amber);
      sight.position.set(0, 0.105, -0.2);
      g.add(slide, frame, barrel, gr, sight);
      return new THREE.Vector3(0, 0.05, -0.34);
    });
    // HORNET SMG
    build((g) => {
      const body = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.1, 0.5), dark);
      body.position.set(0, 0.02, -0.1);
      const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.022, 0.022, 0.24, 8), steel);
      barrel.rotation.x = Math.PI / 2;
      barrel.position.set(0, 0.045, -0.42);
      const mag = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.2, 0.07), steel);
      mag.position.set(0, -0.12, -0.06);
      mag.rotation.x = 0.12;
      const stock = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.07, 0.18), grip);
      stock.position.set(0, 0, 0.2);
      const fore = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.06, 0.16), amber);
      fore.position.set(0, -0.04, -0.28);
      g.add(body, barrel, mag, stock, fore);
      return new THREE.Vector3(0, 0.045, -0.55);
    });
    // MAUL-12 shotgun
    build((g) => {
      const body = new THREE.Mesh(new THREE.BoxGeometry(0.09, 0.11, 0.56), dark);
      body.position.set(0, 0.02, -0.08);
      const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 0.42, 8), steel);
      barrel.rotation.x = Math.PI / 2;
      barrel.position.set(0, 0.05, -0.44);
      const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.028, 0.028, 0.34, 8), steel);
      tube.rotation.x = Math.PI / 2;
      tube.position.set(0, -0.025, -0.38);
      const pump = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.07, 0.14), grip);
      pump.position.set(0, -0.045, -0.3);
      const stock = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.1, 0.22), grip);
      stock.position.set(0, -0.02, 0.22);
      stock.rotation.x = 0.15;
      g.add(body, barrel, tube, pump, stock);
      return new THREE.Vector3(0, 0.05, -0.66);
    });
  ...
  private tryFire() {
    ...
    flash.scale.setScalar(0.8 + w.def.kick * 0.5);
  ...
  private tick() {
    ...
    if (vm) {
      vm.position.set(
        0.34 + Math.sin(this.bobPhase) * 0.008,
        -0.3 + Math.cos(this.bobPhase * 2) * 0.008 + this.vmLift - (this.reloading > 0 ? 0.12 * Math.sin((1 - this.reloading / this.reloadTotal) * Math.PI) : 0),
        -0.62 + this.vmRecoil
      );
